...
import { IdlePlaceholder } from './ui/IdlePlaceholder'
import { useUnitStore } from '../store/unitStore'
import { formatTemp } from '../utils/temperature'
import { formatTime } from '../utils/time'
import { CHAMBER_TEMP_MIN, MODEL_ACCENT } from '../utils/constants'
...
function TempCell({ label, current, target }) {
  ...
  // Heating/cooling logic stays in Celsius
...
export function PrinterCard({ printer, status, dismissed = [] }) {
  ...
  const thumbUrl     = useThumbnailUrl(printer.id)
  ...
  return (
    <>
      ...
      >
        ...
        {/* ── Print thumbnail / idle placeholder ───────────────────── */}
        {isRunning && (st.cover_file || st.gcode_file || st.current_file) ? (
          ...
          />
        ) : (st.gcode_state === 'IDLE' || st.gcode_state === 'FINISH') ? (
          <IdlePlaceholder state={st.gcode_state} />
        ) : null}
